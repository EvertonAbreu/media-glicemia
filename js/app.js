...
// Export PDF - CORRIGIDO PARA SAFARI
...
if (exportBtn) {
    exportBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // Fallback: tentar salvar usando método alternativo
            try {
                // Criar um blob e baixar diretamente
                const blob = new Blob([pdfContent], { type: 'application/pdf' });
                const url = URL.createObjectURL(blob);
                const link = document.createElement('a');
                link.href = url;
                link.download = `diabcare_${new Date().toISOString().slice(0, 10)}.pdf`;
                ...
                // Se tudo falhar, abrir em nova janela
                try {
                    ...
                    if (printWindow) {
                        ...
                        showNotification('Erro ao gerar PDF. Tente usar outro navegador.', 'error');
                    }
                } catch (finalError) {
